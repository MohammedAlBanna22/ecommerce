...
<div class="bg-white rounded-lg border border-amz-border mt-3 overflow-hidden">
    <div class="flex border-b border-amz-border overflow-x-auto">
        <button onclick="switchTab('description')"
            class="tab-btn active px-6 py-3.5 text-[14px] text-amz-text whitespace-nowrap">Description</button>
        <button onclick="switchTab('specifications')"
            class="tab-btn px-6 py-3.5 text-[14px] text-amz-text-sec whitespace-nowrap">Specifications</button>
        <button onclick="switchTab('reviews')"
            class="tab-btn px-6 py-3.5 text-[14px] text-amz-text-sec whitespace-nowrap">Customer Reviews</button>
    </div>
        ...
        <table class="w-full max-w-2xl">
            <tbody>
                ...
                <tr class="spec-row border border-amz-border-light">
                    ...
                    <td class="px-4 py-3 text-[13px] text-amz-text line-through">
                ...
                <tr class="spec-row border border-amz-border-light">
                    ...
                    <td class="px-4 py-3 text-[13px]">
                        <span
                            class="px-2 py-0.5 rounded text-[11px] font-medium {{ $product->status === 'available' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
        ...
        <div class="flex flex-col sm:flex-row gap-8">
            <div class="flex-shrink-0 text-center sm:text-left">
                ...
                <div class="mt-4 space-y-1.5 w-48">
                    @for($bar = 5; $bar >= 1; $bar--)
                    @php $barPct = $bar === 5 ? rand(40,65) : ($bar === 4 ? rand(15,30) : ($bar === 3 ? rand(5,15) :
                    ($bar === 2 ? rand(1,8) : rand(0,5)))); @endphp
                    <div class="flex items-center gap-2">
                        <span class="text-[12px] text-amz-text-sec w-6">{{ $bar }}</span>
                        <svg class="w-3.5 h-3.5 text-amz-star flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path
